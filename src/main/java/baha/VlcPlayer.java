...
public class VlcPlayer {
	...
	public VlcPlayer() {
		if (BahaStater.properties.getProperty(Constants.VIDEOPROP) != null
				&& BahaStater.properties.getProperty(Constants.VIDEOPROP).trim().length() > 0) {
...
//		NativeLibrary.addSearchPath(RuntimeUtil.getLibVlcLibraryName(), BahaStater.properties.getProperty(Constants.VLCPATH));
			try {
				...
				final EmbeddedMediaPlayer emp1 = mpf
						.newEmbeddedMediaPlayer(new Win32FullScreenStrategy(BahaStater.videoFrame));
				...
				BahaStater.videoFrame.addWindowListener(new WindowAdapter() {
					...
					public void windowClosed(WindowEvent e) {
						...
						for (GraphicsDevice gd : GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()) {
							if (!device.equals(gd)) {
								BahaStater.displayFrame.setSize(new Dimension(BahaStater.displayFrameX.intValue(),
										BahaStater.displayFrameY.intValue()));
								BahaStater.displayFrame
										.setLocation(gd.getDefaultConfiguration().getBounds().getLocation());
				...
				progressBarPanel
						.setBackground(Color.decode(BahaStater.properties.getProperty(Constants.MAIN_BACKGROUND)));
				imgPane = new JPanel();
				imgPane.setLayout(
						new GridLayout(0, 4, Double.valueOf(BahaStater.initFrame.getHeight() * .01).intValue(),
								Double.valueOf(BahaStater.initFrame.getHeight() * .01).intValue()));
				imgPane.setBackground(Color.decode(BahaStater.properties.getProperty(Constants.MAIN_BACKGROUND)));
				imgPane.setPreferredSize(new Dimension(Double.valueOf(BahaStater.initFrame.getWidth() * .80).intValue(),
						Double.valueOf(BahaStater.initFrame.getHeight() * .75).intValue()));
				scrollPane = new JScrollPane(imgPane);
				scrollPane.setPreferredSize(
						new Dimension(Double.valueOf(BahaStater.initFrame.getWidth() * .81).intValue(),
								Double.valueOf(BahaStater.initFrame.getHeight() * .76).intValue()));
				...
				progressBar
						.setBackground(Color.decode(BahaStater.properties.getProperty(Constants.SELECTED_BACKGROUND)));
				...
				progressBar.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						// Computes how far along the mouse is relative to the component width then
						// multiply it by the progress bar's maximum value.
...
//								emp1.play();
									...
									JLabel current = null;
									try {
										Color clr = Color.decode(
												BahaStater.properties.getProperty(Constants.SELECTED_BACKGROUND));
										for (int p = 0; p < imgPane.getComponents().length; p++) {
											if (imgPane.getComponent(p) instanceof JLabel
													...
													&& imgPane.getComponent(p).getBackground().getGreen() == clr
															.getGreen()
													&& imgPane.getComponent(p).getBackground().getBlue() == clr
															.getBlue()) {
										...
										Arrays.asList(imgPane.getComponents()).parallelStream().forEach(t -> {
											t.setBackground(Color.decode(
													BahaStater.properties.getProperty(Constants.BLOCK_BACKGROUND)));
										...
										next = (JLabel) Arrays.asList(imgPane.getComponents()).parallelStream()
												.filter(f -> (((JLabel) f).getText())
														.equals(videoFiles.get(currVideoIndex).getName()))
												.findFirst().get();
										next.setBackground(Color.decode(
												BahaStater.properties.getProperty(Constants.SELECTED_BACKGROUND)));
										next.setForeground(Color.decode(
												BahaStater.properties.getProperty(Constants.SELECTED_FONT_COLOR)));
										imgPane.revalidate();
										imgPane.repaint();
									} catch (Exception e) {
				...
				playVideo.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						for (GraphicsDevice gd : GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()) {
							if (!device.equals(gd)) {
								BahaStater.videoFrame.setSize(new Dimension(BahaStater.displayFrameX.intValue(),
										BahaStater.displayFrameY.intValue()));
								BahaStater.videoFrame
										.setLocation(gd.getDefaultConfiguration().getBounds().getLocation());
								break;
							}
						}
...
//				progressBar.setValue(emp1.getLength());
				...
				buttonPanel.setLayout(
						new GridLayout(0, 1, Double.valueOf(BahaStater.initFrame.getHeight() * .02).intValue(),
								Double.valueOf(BahaStater.initFrame.getHeight() * .02).intValue()));
				...
				buttonPanel.setPreferredSize(
						new Dimension(Double.valueOf(BahaStater.initFrame.getWidth() * .10).intValue(),
								Double.valueOf(BahaStater.initFrame.getHeight() * .80).intValue()));
				progressBarPanel.setPreferredSize(
						new Dimension(Double.valueOf(BahaStater.initFrame.getWidth() * .80).intValue(),
								Double.valueOf(BahaStater.initFrame.getHeight() * .80).intValue()));
...
//		displayPanel.add(progressBar, BorderLayout.SOUTH);
				...
				JOptionPane.showMessageDialog(BahaStater.initFrame, "VLC installation is not supported!");
				e3.printStackTrace();
			}
		}
		...
		if (file.isDirectory()) {
			for (File fl : file.listFiles()) {
				if (!fl.isHidden() && fl.isFile()) {
					...
					lblFile.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createBevelBorder(0),
					...
					lblFile.addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent e) {
							...
							lblFile.setBackground(
									Color.decode(BahaStater.properties.getProperty(Constants.SELECTED_BACKGROUND)));
							lblFile.setForeground(
									Color.decode(BahaStater.properties.getProperty(Constants.SELECTED_FONT_COLOR)));
							lblFile.setOpaque(true);
